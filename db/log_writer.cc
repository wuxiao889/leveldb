...
Status Writer::AddRecord(const Slice& slice) {
  ...
  return s;
  // 调用者 根据 option 指定的 sync 决定是否做 log 文件的强制 sync。
}

// 按照格式写记录
// checksum(4) len(2) recordtype(1) record
Status Writer::EmitPhysicalRecord(RecordType t, const char* ptr,
                                  size_t length) {
  assert(length <= 0xffff);  // Must fit in two bytes 32k < 64k
  ...
  if (s.ok()) {
    s = dest_->Append(Slice(ptr, length));  // append record
    if (s.ok()) {
      s = dest_->Flush();
    }
  }
